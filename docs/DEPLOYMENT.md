...
- **Healthcheck**: built into the image, checking two independent things (see `src/cli/health.ts`):
  ...
  - **Delivery path**: did the last attempt to reach the webhook/Logic App succeed, within `DELIVERY_MAX_AGE_SECONDS` (default 7200)? A reachability probe (`CHECKCENTRAL_INTERVAL_MINUTES`, default 60) keeps this signal fresh even during a quiet stretch with no incidents to report — it completes a TLS handshake with the host of `WEBHOOK_URL` and sends **no HTTP request**, so it never starts a Logic App run. (Until v0.5.1 it POSTed an empty `{}` body; the Logic App trigger accepts any method and body, so every probe became a run with an empty envelope.) Only a network-level failure (timeout, DNS, connection refused, TLS error — exactly what broke on 2026-09-20) counts as unreachable. It does not prove the Logic App itself is enabled; real traffic does.
...
| `docker inspect` shows `unhealthy: delivery: ...` | The webhook/Logic App is unreachable, rejecting requests, or the URL/SAS signature is stale | Check logs for "Webhook reachability probe failed" and the underlying network/TLS error, or "Teams JSON payload" errors for real traffic. |
